 ...
 * Version: 0.7.0 - 2014-10-06T03:53:35.349Z
 * License: MIT
 */
...
$templateCache.put("bootstrap/match-multiple.tpl.html","<span class=\"ui-select-match\" ng-model=\"$select.selected\" ui-sortable=\"{{$select.sortableOptions}}\"><span ng-repeat=\"$item in $select.selected\"><a style=\"margin-right: 3px;\" class=\"ui-select-match-item btn btn-default btn-xs btn-flat\" tabindex=\"-1\" type=\"button\" ng-disabled=\"$select.disabled\" ng-click=\"$select.activeMatchIndex = $index;\" ng-class=\"{\'btn-primary\':$select.activeMatchIndex === $index}\"><span class=\"close ui-select-match-close\" ng-hide=\"$select.disabled\" ng-click=\"$select.removeChoice($index)\">&nbsp;&times;</span> <span uis-transclude-append=\"\"></span></a></span></span>");
$templateCache.put("bootstrap/match.tpl.html","<button type=\"button\" class=\"btn btn-default btn-flat form-control ui-select-match\" tabindex=\"-1\" ng-hide=\"$select.open\" ng-disabled=\"$select.disabled\" ng-class=\"{\'btn-default-focus\':$select.focus}\" ;=\"\" ng-click=\"$select.activate()\"><span ng-show=\"$select.searchEnabled && $select.isEmpty()\" class=\"text-muted\">{{$select.placeholder}}</span> <span ng-hide=\"$select.isEmpty()\" ng-transclude=\"\"></span> <span class=\"caret ui-select-toggle\" ng-click=\"$select.toggle($event)\"></span></button>");
